...
class JobScraperLinkedin:
    ...
    def isJobApplied(self, job: WebElement):
        try:
            ...
            if "Applied" in applied.text:
                logger.info("skipping already applied job")
                return True
        except:
            logger.info("job not applied, extracting job data")
            return False

    def processJob(self, job: WebElement, page):
        thread_id = threading.current_thread().name
        if self.limit_reached_event.is_set():
            logger.info(f"limit reached return current job thread: {thread_id}")
            return
        logger.info(f"Current Job Thread Name: {thread_id}")
        ...
        with self.global_job_index_lock:
            ...
            logger.info(f"global job index tracker: {self.global_job_index}")
            if self.global_job_index >= self.application_limit:
                self.limit_reached_event.set()
                logger.info("limit reached, set event to all threads..")
            logger.info(f"current job index: {self.job_index_list[page]}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.job_threads, thread_name_prefix=prefix) as executor:
            ...
            for i, job in enumerate(job_list):
                ...
                if i%self.job_threads == 0:
                    logger.info("current job loop index: %d", i)
    ...
    def searchJobsThreads(self, page_to_visit):
        ...
        thread2 = threading.Thread(target=self.saveJobsListThreaded, args=[
                                   page_to_visit], daemon=False)
        thread2.start()
        thread2.join()
    ...
    def writeDataToCsv(self, Data_in:[dict]  , Csv_file_out):
        ...
        logger.info(f"CSV file '{Csv_file_out}' created successfully.")
    ...
    def sortDataByIndexCsv(self, csvToSort):
        ...
        logger.info(f"CSV file '{csvToSort}' sorted successfully.")
    ...
    def writeJobToCsv(self, Job: dict, Csv_file_out):
        ...
        with open(Csv_file_out, 'a', newline='') as file:
            ...
            logger.info(f"CSV file '{Csv_file_out}' updated successfully.")
            flocker.unlock(file)
